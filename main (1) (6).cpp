...
int main() {
    std::string teamFile =  "C:/Users/sherl/Downloads/teams_csvFile.csv";
    std::string outputCSV = "C:/Users/sherl/Downloads/teams_csvFile.csv";

    std::vector<Team> teams = loadTeams(teamFile);
    if (teams.empty()) {
        std::cout << "No teams found! Check the CSV file." << std::endl;
        return 1;
    }

    std::vector<std::tuple<Team, Team, int, std::string>> schedule;
    makeFixtures(teams, schedule);
    saveToCSV(outputCSV, schedule);

    std::cout << "League Fixtures saved to " << outputCSV << std::endl;
    return 0;
